...
func Parse(m Match, scopes []string, command string, files []string, silent bool, exTag string) Match {
	...
	r1 := regexp.MustCompile(`(\w+\:\/\/)*(([\w\d\-]+|\*.)+\S\.[\.\w\d\-]+)(\:\d+)?(\/\S+)?`)
	// Groups: 1.  (ftp)://sub.example.com:25/d/foo.bar    // protocol
	//	       2.   ftp://sub.(example.com):25/d/foo.bar   // host/domain
	//	       3.   ftp://(sub)example.com:25/d/foo.bar    // sub
	//         4.   ftp://sub.example.com:(25)/d/foo.bar   // port
	//         5.   ftp://sub.example.com:25(/d/foo.bar)   // dir/file
